    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-[1fr_auto]">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/20 backdrop-blur-sm"><ShieldAlert className="h-6 w-6" /></div>
                <div><h1 className="font-display text-2xl font-bold">Breach Check</h1><p className="text-sm text-white/70">Monitor your accounts for data breaches</p></div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="glass-inset-panel px-4 py-3"><p className="text-xs text-muted-foreground">Data Exposure</p><p className="font-display text-lg font-bold text-foreground">{ratingLabel}</p></div>
                <div className="glass-inset-panel px-4 py-3"><p className="text-xs text-muted-foreground">Total Breaches</p><p className="font-display text-2xl font-bold text-foreground">{totalBreaches}</p></div>
                <div className="glass-inset-panel px-4 py-3"><p className="text-xs text-muted-foreground">Avg. Score</p><p className="font-display text-2xl font-bold text-foreground">{avgScore}%</p></div>
              </div>
            </div>
            <div className="flex items-center justify-center"><div className="glass-inset-panel p-4"><RiskGauge score={avgScore} size={150} /></div></div>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={item}>
        <div className="glass-card flex gap-3 p-4">
          <Input placeholder="Add email or account to monitor..." value={newEmail} onChange={(e) => setNewEmail(e.target.value)} className="border-white/10 bg-transparent" />
          <Button className="shrink-0"><Plus className="mr-2 h-4 w-4" /> Add</Button>
        </div>
      </motion.div>
