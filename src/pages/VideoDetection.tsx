...
import { Upload, Video, AlertTriangle, Clock } from "lucide-react";
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Upload a video to analyze for deepfake indicators</p>
      </div>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-sm">
                  <Clock className="h-4 w-4 text-cyber-purple" /> Detection Timeline
                </CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  {result.timeline.map((t, i) => (
                    <div key={i} className={`flex items-start gap-3 rounded-lg p-3 ${severityBg(t.severity)}`}>
                      ...
                      <div>
                        ...
                        <span className={`text-xs font-medium uppercase ${severityColor(t.severity)}`}>{t.severity} severity</span>
                      </div>
                    </div>
                  ))}
                </div>
