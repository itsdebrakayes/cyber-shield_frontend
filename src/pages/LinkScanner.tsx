          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            <Card>
              <CardContent className="flex flex-col items-center py-8">
                ...
                <p className={`mt-2 font-display text-2xl font-bold ${getVerdictColor(result.verdict)}`}>
                  {result.verdict}
                </p>
                <p className="mt-1 flex items-center gap-1 text-sm text-muted-foreground">
                  <ExternalLink className="h-3.5 w-3.5" /> {result.url}
                </p>
              </CardContent>
            </Card>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="flex items-center gap-2 text-sm">
                    <Shield className="h-4 w-4 text-cyber-blue" /> Google Safe Browsing
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-lg font-bold text-score-danger">{result.googleSafeBrowsing.status}</p>
                  <p className="text-xs text-muted-foreground">Safety score: {result.googleSafeBrowsing.score}/100</p>
                </CardContent>
              </Card>
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="flex items-center gap-2 text-sm">
                    <Shield className="h-4 w-4 text-cyber-purple" /> VirusTotal
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-lg font-bold text-score-warning">
                    {result.virusTotal.detected}/{result.virusTotal.total} engines flagged
                  </p>
                  <p className="text-xs text-muted-foreground">Safety score: {result.virusTotal.score}/100</p>
                </CardContent>
              </Card>
            </div>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-sm">
                  <AlertTriangle className="h-4 w-4 text-score-danger" /> Flags & Warnings
                </CardTitle>
              </CardHeader>
              <CardContent>
                <ul className="space-y-2">
                  {result.flags.map((flag, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm">
                      <XCircle className="mt-0.5 h-4 w-4 flex-shrink-0 text-score-danger" />
