...
import { Upload, Mic, Clock } from "lucide-react";
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="flex items-center gap-2 text-sm"><Clock className="h-4 w-4 text-cyber-teal" /> Detected Anomalies</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  {result.anomalies.map((a, i) => (
                    <div key={i} className={`flex items-start gap-3 rounded-lg p-3 ${severityBg(a.severity)}`}>
                      ...
                      <div>
                        ...
                        <span className={`text-xs font-medium uppercase ${severityColor(a.severity)}`}>{a.severity}</span>
                      </div>
                    </div>
                  ))}
                </div>
